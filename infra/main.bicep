...
@description('Optional existing Azure Monitor action group resource ID for availability alerts.')
param availabilityAlertActionGroupId string = ''
...
module availability 'modules/availability.bicep' = if (deployApp) {
  name: 'availability'
  scope: resourceGroup
  params: {
    name: 'avail-ats-${environmentName}-${suffix}'
    applicationInsightsId: observability.outputs.applicationInsightsId
    healthUrl: 'https://${app!.outputs.fqdn}/health'
    actionGroupResourceId: availabilityAlertActionGroupId
    tags: resourceGroup.tags
  }
}
